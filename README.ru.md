...
- **Библиотека** — вторая вкладка с записями, которые вы сохраняете сами: тексты и файлы,
  они не вытесняются и разложены по папкам.
...
| `Tab` | переключить «История / Библиотека» |
| `Ctrl+S` | сохранить выбранную запись в библиотеку |
| `F2` | изменить запись библиотеки (название, текст, папка) |
| `Esc` | закрыть окно |
...
История — это лента: старые записи вытесняются новыми. Вкладка **«Библиотека»** хранит то,
что вы сохранили осознанно: эти записи не вытесняются и живут между запусками.

- **Папки** слева, «Все» показывает всё сразу.
- **`+ Текст`** создаёт именованную заготовку, **`+ Файл`** копирует файлы внутрь, либо
  просто **перетащите файлы в окно**.
- **`Ctrl+S`** на вкладке «История» переносит выбранную запись в библиотеку.
- Файлы **копируются** в `%APPDATA%\WiClip\library\`, поэтому перемещение или удаление
  оригинала ничего не ломает. Вставка отдаёт настоящие файлы — в проводник, в почту,
  в мессенджер. Файлы больше 100 МБ не принимаются.
- **`F2`** правит название, текст и папку, **`Shift+Delete`** удаляет запись.
- Кнопка **📌** в заголовке не даёт окну закрываться — нужна для перетаскивания файлов,
  иначе окно прячется, как только вы кликаете в другую программу.
...
Результат: `dist\WiClip-1.1.0-x64.msi` и папка `publish\x64` с готовым приложением.
...
- `library.json` и `library\` — библиотека и скопированные в неё файлы;
...
src/WiClip/
  ...
  LibraryStore.cs        библиотека: папки, записи, копии файлов
  LibraryItem.cs         модели записи и папки библиотеки
  ItemEditorWindow.xaml  создание и правка записи библиотеки
